...
class Client extends EventEmitter {
    ...
    async initialize() {
        ...
        if (this.options.session) {
            ...
            const getQrCode = async () => {
                ...
                const qrImgData = await page.$eval(
                    QR_CANVAS_SELECTOR,
                    (canvas) =>
                        [].slice.call(
                            canvas.getContext("2d").getImageData(0, 0, 264, 264)
                                .data
                        )
                );
            ...
            getQrCode();
            this._qrRefreshInterval = setInterval(
                getQrCode,
    ...
    async acceptGroupV4Invite(inviteInfo) {
        ...
        return await this.pupPage.evaluate(async (inviteInfo) => {
            let { groupId, fromId, inviteCode, inviteCodeExp, toId } =
                inviteInfo;
            return await window.Store.Wap.acceptGroupV4Invite(
                groupId,
                fromId,
                inviteCode,
